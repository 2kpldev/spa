...
<body class="ms-body ms-aside-left-open ms-primary-theme ms-has-quickbar">
  ...
  <main class="body-content">
    ...
    <?php
      $get_resultset=$_SQL($con,"SELECT*FROM spa_category WHERE cateId='$_GET[id]'");
      $result=$_ARRAY($get_resultset);
      ?>
    <!-- Body Content Wrapper -->
                ...
                <div class="col-xl-12 col-md-12 pd-0">
                  <form action="#" method="post">
                    <div class="form-group d-flex m-0 fs-14 clearfix">
                     <input type="hidden" class="form-control fs-20"
                      name="cateId" value="<?php echo $_GET['id']; ?>" required>
                      <div class="input-group">
                      <input type="text" class="form-control mr-2" name="cateName" value="<?php echo $result['cateName']?>" required>
                      <button type="submit" name="onSubmit" class="btn-sm btn-outline-primary float-right">
                        <i class="fa fa-check-circle"> </i> ຢຶນຢັນ
                      </button>
                    </div>
                    </div>
                  </form>
...
if(isset($_POST['onSubmit'])){
  ...
  $created=$_SQL($con,"UPDATE spa_category SET cateName='$cateName' WHERE cateId='$cateId'");
  if($created){
